  ...
  return (
    <div className="min-h-screen bg-gray-900 text-white flex flex-col items-center justify-center p-4">
      ...
      <div className="bg-gray-800 p-6 rounded-lg shadow-lg text-center">
        <p className="text-lg font-semibold mb-2">Balance: <span className="text-green-400">${balance}</span></p>
        <input
          type="number"
          value={betAmount}
        ...
        <button
          onClick={rollDice}
          className="mt-4 bg-magenta-600 hover:bg-magenta-700 text-white font-bold py-2 px-4 rounded w-full"
          disabled={loading}
        >
        ...
        {hash && <p className="mt-2 text-sm break-all text-gray-400">Hash: {hash}</p>}
      </div>
    </div>
  );
}
